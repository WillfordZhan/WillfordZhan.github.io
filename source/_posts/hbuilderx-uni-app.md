...
3. 微信开发者工具最好先打开 **服务端口**
4. 如果微信开发者工具里项目详情的 `AppID` 显示为空，手动导入时要 **手填 AppID**
...
但如果你在微信开发者工具里打开的是一个旧项目实例，或者是以错误方式导入的项目，微信开发者工具当前运行的项目依然可能是“无 AppID”状态。
...
7. 删除旧项目，手动导入 `unpackage/dist/build/mp-weixin`
8. 导入时手动填写 AppID
9. 再看项目详情里的 `AppID` 是否为空
...
**`uni-app` 联调时，先确认产物有没有重新生成，再确认微信开发者工具里当前项目的 AppID 不是空。**
