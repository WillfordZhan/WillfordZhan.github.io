...
- 方案已简化：Python 侧仅保留一个配置入口 `MCP_API_TOKEN`，默认值固定为 `<demo-mcp-token>`，移除复杂 fallback 链路，减少歧义与隐式行为。
...
- 配置入口：`MCP_API_TOKEN`
- 默认值：`<demo-mcp-token>`
- 行为：若未显式设置环境变量，统一走默认占位值；若设置了 `MCP_API_TOKEN`，严格使用该值。
...
```bash
curl -sS -X POST 'http://127.0.0.1:19001/mock-mcp/tools/list' \
  -H 'Content-Type: application/json' \
  -H 'X-INTERNAL-MCP-TOKEN: <demo-mcp-token>' \
  -d '{}'

curl -sS 'http://127.0.0.1:18000/system/mock-mcp/status?refresh=true'
```

结果要点：

- `tools/list` 返回 `code=200`
- `system/mock-mcp/status` 返回：
  - `connected=true`
  - `tool_count=10`
  - `detail="ok (tools=10)"`
...
- 结论、命令、结果均已固化在本工作日志。
- 验证入口固定：`/system/mock-mcp/status?refresh=true` 与 `tests/test_mcp_client.py`。
...
2. 恢复旧配置逻辑后，立即执行：
   - `pytest tests/test_mcp_client.py`
   - `curl .../system/mock-mcp/status?refresh=true`
