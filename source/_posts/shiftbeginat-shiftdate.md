---
title: "踩坑复盘：ObjectMapper 被覆盖引发 shiftBeginAt/shiftDate 连锁异常"
...
这次排障最核心的坑，不是某个 DTO 字段写错，而是 **`ObjectMapper` 被覆盖后出现多实例分叉**。  
`shiftBeginAt` 返回数组、`shiftDate` 反序列化报错，是同一个根因触发的两个表象。
...
根因不是“Jackson 随机异常”，而是 **`ObjectMapper` 被覆盖，导致序列化/反序列化配置失去统一入口**：
