...
- 看 SQL 日志
- 配连接池参数
- 用 `SHOW PROCESSLIST` 做基础排查

但还没有把下面这些知识串成一条完整因果链，那么这次踩坑很值得认真看完：

- MySQL metadata lock 到底怎么影响线上查询
- 为什么 DDL 等待会拖住后续读请求
- 为什么 JDBC 最终会把锁等待包装成“通信失败”
- 为什么 Java + Spring + Druid 默认不容易复现，但 Python 或手写 JDBC 更容易踩中
- 为什么“只读查询不收尾事务”会成为隐形 blocker

## 现场时间线：现象是怎么一步步放大的

这次故障不是一个点炸出来的，而是一串条件叠加后暴露的：
...
如果总是稳定在 4 秒左右，很大概率不是随机网络抖动，而是某个明确的超时配置触发了。

信号二：SQL 是普通查询。

普通查询在固定阈值上报通信失败，优先应该怀疑：
...
所以 JDBC 看到的并不是“数据库主动拒绝你”，而是“服务端迟迟没有新数据包回来”。这就是为什么最终异常长得像：
...
这条信息一出现，方向基本就已经从“网络问题”切到“锁问题”了。
...
这个现象值得单独展开，因为很多开发工程师只知道“DDL 会被长事务挡住”，但不知道“后续读为什么也会被挡住”。
...
这类代码平时往往不报错，因为查询能正常返回。真正可怕的是它会在后续 DDL、锁等待、连接复用时把问题放大。

## 修复示范：为什么只读查询后显式 `rollback()` 是对的

这次 Python 侧修复的核心不是“改性能”，而是“给只读事务一个明确结束点”。
...
对手动事务模型来说，这其实是很标准的工程做法。

## `autocommit=True` 能不能一把梭解决

很多人看到这里会自然问一个问题：那是不是直接 `autocommit=True` 就好了？
...
这叫“最小变更面修根因”，而不是“大开关式改行为”。

## 典型反例代码：为什么 `SELECT ... FOR UPDATE` 会放大锁竞争

除了只读事务不收尾，这次还顺手处理了另一个不必要的放大器：事件追加路径里的显式行锁。
...
所以默认更不容易形成“历史连接 Sleep 很久但事务还活着”的状态。
...
这类手写 JDBC 风格，本质上和 Python 那个问题没有区别。
...
在我看来，真正进阶的做法不是只会 kill 会话，而是会把现场止血和后续治理分开：
...
## 延伸问答：这次排障里最值得追问的几个问题

下面这些问题，都是这次复盘里非常有价值的追问。它们之所以重要，不是因为问题本身难，而是因为每一个问题都逼着我们把“现象、机制、修复”真正串起来。

### 追问 1：Python 这次改动的目的到底是什么

Python 侧这次改动的核心目标，不是“让查询更快”，也不是“顺手优化一下代码风格”，而是要解决一个非常具体的问题：
...
这就是为什么修复点不是改 SQL，而是给读路径补事务收尾。

### 追问 2：为什么这个改动可以防止再次发生

要回答这个问题，关键是把根因写成一个完整句子：

> 这次事故的一类 blocker，来自“旧连接进入 Sleep，但背后事务没有结束”。

那么只要修复动作能稳定打断这条链，问题就会显著下降。
...
这不是拍脑袋推理，而是标准数据库会话模型：

- 事务不结束，锁语义就可能继续存在
- 事务一旦结束，相关锁上下文才会被释放

换句话说，这次修复不是“碰巧好了”，而是直接对准了根因链条中最关键的一环。
...
所以真正重要的不是语言，而是连接和事务的组织方式。

### 追问 6：这次经验对开发工程师真正有价值的点是什么

我觉得最值得带走的，不是某一条 MySQL 知识点，而是下面这条排障思维：
...
这也是为什么我认为，这类故障非常适合面向开发工程师做进阶复盘。它把数据库、驱动、连接池、事务边界和工程写法真正串成了一张图。
